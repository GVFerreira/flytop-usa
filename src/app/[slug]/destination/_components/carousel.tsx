...
export function SynchronizedCarousel({ imageSlideFromDB, imageThumb }: CarouselProps) {
  ...
  return (
    <div>
      ...
      >
        <SplideSlide>
          <Image src={imageThumb} alt="Thumbnail" width={1024} height={1024} quality={70} priority className="aspect-square object-cover w-full rounded-xl border-2 border-slate-900" />
        </SplideSlide>
        {images.map((imageUrl: string, index: number) => (
          <SplideSlide key={index}>
            <Image src={imageUrl} alt={`Slide ${index + 1}`} width={1024} height={1024} quality={70} className="aspect-square object-cover w-full rounded-xl border-2 border-slate-900" />
          </SplideSlide>
        ))}
      </Splide>
