...
import BuyNowDrawer from "./_components/buy-now-drawer"
...
export default async function Destination({ params }: Props) {
  try {
    ...
    const rawDestination = await getDestination(params.slug)

    // Properly serialize the data to ensure it's safe to pass to client components
    // This ensures we only pass plain objects, not classes or complex objects
    const destination = rawDestination ? JSON.parse(JSON.stringify(rawDestination)) : null

    if (destination) {
      // Convert the ID to a string to ensure it's serializable
              ...
              <div className="container grid grid-cols-1 md:gap-20 gap-10 md:grid-cols-2 items-start">
                ...
                <div className="bg-[#FFFEEE] flex flex-col gap-y-4 px-6 md:px-12 py-5 md:py-10 rounded-xl shadow-lg">
                  ...
                  <div className="flex flex-row gap-x-4 justify-start items-center">
                    ...
                    <ShareButton
                      title={destination.name}
                      url={`https://flytoptravels.com/${destination.slug}/destination`}
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    <h2 className="text-lg">
                      Airport destination: {destination.name} ({destination.destinationAirport})
                    </h2>
                    <h2 className="text-lg">
                      Airport origin: {destination.departureCity} ({destination.departureAirport})
                    </h2>
                  </div>
                  <p className="flex flex-row justify-start items-center gap-x-2">
                    <span>Flight company:</span>
                    <Image
                      src={destination.company.imagePath || "/placeholder.svg"}
                      width={150}
                      height={75}
                  ...
                  <div>
                    <p className="text-4xl text-green-800 font-bold mb-2">
                      ...
                      {destination.price?.toLocaleString("en-US", {
                    ...
                    <p className="text-lg line-through text-sky-950 font-bold mb-2">
                      ...
                      {destination.regularPrice?.toLocaleString("en-US", {
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div dangerouslySetInnerHTML={{ __html: destination.departureDates }}></div>
                    </div>
                    <div>
                      <span className="text-lg font-bold">Return date:</span>{" "}
                      <div dangerouslySetInnerHTML={{ __html: destination.returnDates }}></div>
                    </div>
                  </div>
    ...
    return (
      <>
        ...
        <main style={{ flex: "1 1 0" }} className="flex flex-col justify-center items-center">
          <h1 className="text-4xl leading-[3rem] text-sky-950 font-bold mb-6 text-center md:text-6xl md:leading-[4.3rem]">
    ...
    return <h1>{e.message || "An error occurred"}</h1>
  }
}
